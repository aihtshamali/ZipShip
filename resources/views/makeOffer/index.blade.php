...
<div class="container profile animate-left">
    <div class="marginauto border width70 padding1per backgrounwhite clear">
        <b><span class="textpurple">Earn more money</span> by delivering more than a single order in a trip</b>
    ...
    <div class="marginauto border width70 padding1per backgrounwhite">
        <img src="{{asset('uploads/userprofile/'.$order->dp)}}" class="circle" width="35px" />
        <b class="paddingLef1per"><span class="black capitalize">{{$order->firstname}} {{$order->lastname}}</span><span class="grayee"> requested</span><span class="right">Deliver Before - {{$order->deliver_before or 'No Date'}}</span></b>
        <div class="marginper fullwidth marginTopTen" style="min-height: 350px;">
            <div class="backgrounwhite width30 left">
                <center>
                    <img src="{{asset('uploads/postImages/'.$order->image_1)}}" width="80%"/>
                </center>
            </div>
            <div class="width68 right padding1per">
                <h3 class="nopad-nomar">{{$order->title}}</h3>
                <p class="backgrounpurpleopp padding3per borderRad7px">
                    <span class="black">Deliver to</span> <span class="white capitalie">{{$order->to_loc}} {{$order->to_city}} , {{$order->to_country}}</span>
                ...
                @if(Auth::id()!=$order->user_id)
                  <div name="" data-id="{{$order->id}}" class="right padding3per myBtn borderRad7px pointer border onhoveroshadow" style="background-color:transparent;color:#800080">
                      <center>Make offer</center>
                  </div>
                @endif
                <div class="clear">
                    <b class="left grayee">item price:${{$order->item_price}}</b>
                    <b class="right grayee">item price:${{$order->item_price}}</b>
                </div>
            </div>
        </div>
  ...
  <!-- The Modal -->
    <div id="myModal" class="modal">
      ...
      <div class="modal-content container width30">
        ...
        <form class="" method="POST" action="{{route('bids.store')}}">
          {{ csrf_field() }}
          <input type="hidden" name="post_id" >
          <div class="form-group">
              <center><h4 style="margin-top: 0px !important;">Bidding Section</h4></center>
          <label class="grayee capitalize" style="font-size: 17px;">Add your desired amount</label>
            <input type="number" name="bidding_amount" step="0.1" placeholder="Bidding amount in US $" class="form-control" value="" required>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
              <span class="right onhoveroshadow"><input type="submit" name="bid" class="btn bg-colored white" value="Bid"></span>
          </div>
      </form>
      </div>
